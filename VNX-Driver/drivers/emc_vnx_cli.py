...
class EMCVnxCli():
    ...
    def extend_volume(self,volume, new_size):
        ...
        #defining CLI command
        command_to_execute = (_('%(navisecclicmd)s lun -expand -name '
        '%(volumename)s -capacity %(newsize)s -sq gb -o -ignoreThresholds ')
            % {'navisecclicmd':self.navisecclicmd,
               'volumename':volumename,
               'newsize':new_size})
        ...
        LOG.debug(_('Extend Volume: %(volumename)s  Return code: %(rc)lu')
                  % {'volumename': volumename,
                     'rc': command_rc})
        if command_rc == 24832:
            LOG.error(_('The LUN cannot be expanded or shrunk because'
                'it has snapshots'))
            LOG.error(_('Command to extend the specified volume failed'))
            raise exception.Error()
        if command_rc != 0:
            LOG.error(_('Command to extend the specified volume failed'))
            raise exception.Error()
    ...
    def _get_host_lun_id(self,host_lun_id_list):
        ...
        existing_hlu_set = set(host_lun_id_list)
        ...
        if largest_hlu != (self.max_luns-1):
            return (largest_hlu+1)
        
        #Search for an unassigned number
            ...
            if not device_number:
                ...
                if lun_map:
                    ...
                    if len(host_lun_id_list) == self.max_luns:
                        LOG.error(_('The storage group has reached the '
                                        'maximum capacity of LUNs'))
                        LOG.error(_('Command to add LUN for volume - %s '
                    ...
                    if host_lun_id == None:
                        LOG.error(_('Unable to get new host lun id.'
                        'Please check if the storage group can accomodate new LUN'))
                        LOG.error(_('Command to add LUN for volume - %s '
                            'in storagegroup failed')%(volumename))
                        raise exception.Error()                
                else:
                ...
                if command_rc ==0:
                    return host_lun_id
                if command_rc ==16896:
                    LOG.error(_('storagegroup command failed:'
                        'Requested Host LUN Number already in use'))
                count += 1
            else:
                LOG.debug(_('LUN was already added in the storage group'))
